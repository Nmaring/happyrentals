...
export const SetupPage: React.FC = () => {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try { await setupCreate(email, password, fullName || undefined); nav("/login"); }
    catch (e:any) { setErr(e?.response?.data?.detail || "Setup failed."); }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
        ...
        <p className="text-sm text-slate-600 mt-1">Create the first admin user.</p>
        ...
        <form className="mt-5 space-y-3" onSubmit={onSubmit}>
          ...
          <div>
            ...
            <input className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" value={fullName} onChange={(e)=>setFullName(e.target.value)} />
          </div>
          <div>
