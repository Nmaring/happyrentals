...
def create_property(payload: schemas.PropertyCreate, db: Session = Depends(get_db)):
    ...
    # Local-mode: satisfy NOT NULL properties.owner_user_id
    if "owner_user_id" not in data or data.get("owner_user_id") is None:
        data["owner_user_id"] = 1
    # Auto-assign owner_user_id in local mode
    data.setdefault("owner_user_id", 1)
...
def delete_property(property_id: int, db: Session = Depends(get_db)):
    ...
    if not obj:
        raise HTTPException(status_code=404, detail="Property not found")
    # Prevent deleting property that still has units
    has_unit = db.execute(text("SELECT 1 FROM units WHERE property_id = :pid LIMIT 1"), {"pid": property_id}).fetchone()
    if has_unit:
        raise HTTPException(status_code=409, detail="Cannot delete property: it still has units. Delete units first.")
    # Prevent deleting property that still has units
    has_unit = db.execute(text("SELECT 1 FROM units WHERE property_id = :pid LIMIT 1"), {"pid": property_id}).fetchone()
    if has_unit:
        raise HTTPException(status_code=409, detail="Cannot delete property: it still has units. Delete units first.")
