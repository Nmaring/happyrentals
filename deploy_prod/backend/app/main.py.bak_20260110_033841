...
# --- Serve built frontend (SPA) from app/static ---
SPA_STATIC_DIR = os.path.join(os.path.dirname(__file__), "static")
if os.path.isdir(SPA_STATIC_DIR):
    assets_dir = os.path.join(SPA_STATIC_DIR, "assets")
    if os.path.isdir(assets_dir):
        app.mount("/assets", StaticFiles(directory=assets_dir), name="assets")

    @app.get("/")
    def spa_root():
        return FileResponse(os.path.join(SPA_STATIC_DIR, "index.html"))

    @app.get("/{path:path}")
    def spa_fallback(path: str):
        # do not swallow API paths
